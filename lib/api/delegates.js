...
module.exports = function (app) {
    ...
    this.getNextForgers = function (error, success) {
        var next_forgers = new NextForgers();
        var forgers = [];
        request.get({
            url : app.get('lisk address') + '/api/delegates/getNextForgers?limit=10',
            json : true
        }, function (err, response, body) {
            if (err || response.statusCode !== 200) {
                return error({ success : false, error : (err || 'Response was unsuccessful') });
                ...
                if (body.delegates) {
                    _.each(body.delegates, function (d) {
                        forgers.push ({publicKey : d});
                    });

                    async.forEach(forgers, function (forger, cb) {
                        next_forgers.getDelegate(forger, cb);
                    }, function () {
                        return success({ success : true, delegates : forgers });
                    });
                }
            } else {
                return error({ success : false, error : body.error });
